...
class HelpSystem:
    ...
    def show_full_help(self,hero):
        ...
        print(
            f"{self.YELLOW}В игре есть девять активных действий.Вводить нужно только первую букву(есть исключения) команды:\n"
            f"""- в (вперед)            - н (назад)                  - у (увернуться)\n- а (атака мечом)       - с (стрельба из ружья)      - о (осмотр-поиск)\n- п (помощь-справка)    - р (рюкзак)                 - искры(выстрел дробью){self.RESET}{self.YELLOW_STAR_END}""")
