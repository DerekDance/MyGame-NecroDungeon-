...
# Создание снаряда
skull = modifiers.create_necromancer_skull_projectile(hero)
print(f"[DEBUG] Создан череп ID={id(skull)}, active={skull.active}")
hero.add_modifier(skull)

# (5.1) Код отвечающий за работу способности Некроманта 'Реверс-поступь' в 1-ой фазе:
revers = modifiers.create_necromancer_revers_move(target=hero)
hero.add_modifier(revers)
# ------------------------------------
while hero.hero_health > 0:

    print(f"[DEBUG] Активных модификаторов: {len(hero.modifiers)}")
    for mod in hero.modifiers:
        print(f"  - {mod.display_name}")
    for mod in getattr(hero, 'active_modifiers', []):
        if isinstance(mod, Projectile) and "череп" in mod.display_name.lower():
            if mod.active and mod.remaining_duration > 0:
                print(f"{hp.YELLOW}[👁️] Череп приближается! Осталось ходов: {mod.remaining_duration}{hp.RESET}")
            elif not mod.active and mod.remaining_cooldown_turns > 0:
                # Опционально: намёк на готовящуюся угрозу
                pass
    # Найдите тот же самый объект
    for mod in hero.modifiers:
        if id(mod) == id(skull):
            print(f"[DEBUG] Череп ID={id(mod)}: active={mod.active}, cd={mod.remaining_cooldown_turns}")
    try:
        # Наказание за долгое бездействие игрока
        ...
        if hero.reverse_step_active:
            if action_hero == "в":
                action_hero = "н"
                print(
                    f"{hp.START_TIRE}{hp.PURPLE}Некромант -*зловещая усмешка* Хочешь идти 'вперёд'? Как насчёт ШАГА НАЗАД?!"
                    f"{hp.RESET}")
            elif action_hero == "н":
                action_hero = "в"
                print(
                    f"{hp.START_TIRE}{hp.PURPLE}Некромант -*зловещая усмешка* Ты сказал 'назад'? Отлично... ИДЁМ ВПЕРЁД!"
                    f"{hp.RESET}")
        # Если заклинание "Реверс-поступь" не действует
        elif not hero.reverse_step_active:
            if action_hero == "в" and necromancer.distance == 1: # ЗДЕСЬ НЕОБХОДИМО ДОБАВИТЬ УСЛОВИЯ ПОСЛЕ НАПИСАНИЯ ОТТАЛКИВАЮЩЕГО УДАРА!
                necromancer.distance -= 1
                print(
                    f"{hp.START_TIRE}{hp.PURPLE}Некромант - 'Надумал обойти меня негодяй?!'"
                    f"{hp.RESET}{hp.info_room(hero.hero_health, hero.hero_max_health, [necromancer])}{hp.END_TIRE}")
            elif action_hero == "в" and necromancer.distance == 2:
                print(
                    f"{hp.START_TIRE}Вы двигаетесь навстречу, а он в свою очередь, усмехаясь делает шаг назад, с кинжалом наготове."
                    f"Так долго с ним можно кружить по комнате."
            ...
            # (5.1) Секретный способ пройти игру #
            elif action_hero == "а" and necromancer.distance < 1: # ЗДЕСЬ НЕОБХОДИМО ДОБАВИТЬ УСЛОВИЯ ПОСЛЕ НАПИСАНИЯ ОТТАЛКИВАЮЩЕГО УДАРА!
                necromancer.distance += 3
                flag_winner = True
                fast_as_hermes += 1
                achievements_system.add_completed_location("(5)  Древняя гробница и Некромант", 2)
                ...
                if flag_winner:
                    ...
                    if fast_as_hermes == 5:
                        ...
                        sys.exit()
            # Если не выполнить условия секретного прохождения
            elif necromancer.distance < 1: # ЗДЕСЬ НЕОБХОДИМО ДОБАВИТЬ УСЛОВИЯ ПОСЛЕ НАПИСАНИЯ ОТТАЛКИВАЮЩЕГО УДАРА!
                necromancer.distance += 3
                hero.hero_health -= 2
                if skull_distance < 3:
                    skull_distance = 1
        ...
        # (5.1) Команда "о"
            ...
            nЕго дистанция примерно: {random.randint(skull_distance-1,skull_distance+1)} или {random.randint(skull_distance-1,skull_distance+1)}{hp.END_TIRE}")
        elif action_hero == "о":
            print(f"{hp.START_TIRE}Быстро оглядевшись, угрозу для себя вы не обнаружили кроме {hp.PURPLE}Некроманта{hp.RESET} "
                  f"напротив.{hp.info_room(hero.hero_health,hero.hero_max_health,[necromancer])}{hp.END_TIRE}")
